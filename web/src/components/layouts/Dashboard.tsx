...
export default function DashboardLayout() {
    ...
    const createInstance = async () => {
        ...
        setCreating(true);
        try {
            const result = await api.createInstance(token);

            if (result.success) {
                // Les instances seront mises à jour automatiquement via WebSocket
                // mais on force un refresh pour être sûr
    ...
    return (
        <div className="min-h-screen bg-bg-dark text-white">
            ...
            {/* Sidebar */}
            <aside className="fixed top-16 left-0 bottom-0 w-64 bg-white/5 backdrop-blur-xl border-r border-white/10 z-40 overflow-y-auto">
